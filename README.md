...
|---|---|---|---|---|
| **Nadine Merabi — Nina Gold** | **1.8M views · 24 creators** *(brand/occasion-level)* | on ShopMy ([product](https://shopmy.us/shop?query=nadine+merabi+dress+nina+gold&tab=popular)) | **6 Nina Gold rentals** ($60–119) | **High-value convergence** → strongest play; onboard the creators |
| **Réalisation Par — The Cora** | 245K views · 10 creators | **34.7K lifetime clicks · 991 promoters** *(recent ≈ 0)* | **76 of 724** | **Convergence** → match renters/owners now (proven demand, now cooling) |
| **House of CB — The Sculpt** | **0 confirmed** — its 4.9M was 100% Pilates | — | 0 of 693 | **Signal contamination** → the homonym trap; real demand needs brand-anchored/LLM capture |
...
- **Nadine Merabi — the "Nina Gold" dress** — the signature gold sequin SKU of a British "wedding-guest" event label (~$300–600 retail); **6 gold copies rentable on Pickle ($60–119)** (of 10 Nina listings across colorways), [product page](https://www.us.nadinemerabi.com/products/nina-gold-dress). *The radar discovered this brand on its own (see below) and I promoted it into the watchlist.*

> Note: the Pickle supply counts (0 / 76 / 6) are **style-specific** — the denominator is each brand's **actual** listings (693 / 724 / 231; Pickle's brand pages mix in other labels — the Nadine Merabi page is only 38% Nadine Merabi — so I brand-filter it, see [density](#supply-density--where-it-actually-is-size--city)), while the numerator matches the style name across the **full page**, which recovers listings whose brand field was mis-parsed (typos, "SOLD OUT" prefixes). They **wobble a listing or two between scrapes** (page rotation — the `diff.py` guard exists to catch that, not real churn).
...
| **Rental liquidity** | **6 Nina Gold copies** ($60–119) | **76 of 724** ($50–90/wk) — deep | 0 of 693 |
...
**Two things TRACE can act on:** (1) supply is **NYC-first** across all three (42–69%) — so a presale/matching launch should start in NYC metro, where renter *and* owner liquidity already co-locate. (2) Supply skews hard to **S/XS (63–71%)** — so M+ renters are structurally underserved, a concrete gap to recruit owners against. (The Nina Gold's 6 gold copies mirror this: XS·S·S·S·6·8, half in NYC metro, rents $60–119 — generated by `density.py`, not hand-counted.)

**A second Pickle finding, free from this:** the brand pages are **contaminated** — the "Nadine Merabi" page is only **38% Nadine Merabi** (231 of 603; the rest are Retrofete, Arcina Ori, House of CB…), and even the cleaner pages are 87–90%. So the honest supply denominator is the **brand-filtered** count, not the page count — which is why `score.py`/`dashboard.py` now filter by brand first (`match.brand_filter`). Same class of data-quality catch as the rotating-subset discovery.

## Recommendation for TRACE
1. **Lead with Nadine Merabi's Nina Gold — the top *real* demand.** 1.8M brand-anchored views, 24 creators, and 6 rentable copies ($60–119) → the strongest all-round play. Onboard the 24 discovered creators to light up ShopMy and match wedding-guest renters to owners now.
...
- **Engineering hygiene.** Scoring (`norm` / `gap_factor` / `trend`) lives once in `score.py`; `dashboard.py` imports it, so the two surfaces can't drift. Dates are parsed from filenames, not `mtime` (identical after a CI checkout). Deps are pinned (`requirements.txt`). And `match.py` — the signal-validity core — has tests (`tests/test_match.py`) that run in CI ([`.github/workflows/test.yml`](.github/workflows/test.yml)).
...
1. **Inverse-search discovery (`discover.py`) — the headline.** Instead of checking a known watchlist, it scrapes trending fashion hashtags (`#fashionhaul`, `#grwm`, `#weddingguestdress`…), extracts brand/product mentions from captions, and surfaces *emerging* dresses **not on any watchlist**. A live run flagged **Nadine Merabi (one post at ~9.2M views) as 🆕 NEW** — which I then **promoted into the watchlist and validated end-to-end** (1.8M brand views, 24 creators, signature Nina Gold SKU with 6 rentable copies). The very next run shows it as `(tracked)` and surfaces the *next* wave — **Norma Kamali (2.6M), Cult Gaia (1.1M)** (plus retailers like Revolve, which the extractor flags but a brand/retailer filter would drop). This is the "alert TRACE when *any* dress goes viral" engine: **discovery, not just monitoring — and the discover→track loop closing in one build.**
...
|---|--:|--:|--:|--:|--:|--:|
| Nadine Merabi — Nina Gold | 95.2 | 33.3 | 1,756,181 | 5,003 | 24 | 231 (6) |
| Réalisation Par Cora | 37.4 | 5.6 | 245,891 | 1,726 | 10 | 724 (76) |
| House of CB Sculpt | 0.0 | **unmeasured** | 0 | 0 | 0 | 693 (0) |

*Trend = 0.5·views + 0.3·saves + 0.2·freshness (min-max normalized). Opportunity = Trend × supply-gap — so dresses that already have supply (Cora, Nadine Merabi) score low on **Opportunity** by design: they're **monetize-now**, not build-the-market. **Sculpt's Opportunity now reads `unmeasured`, not 0** — because its demand is 100% Pilates in our sample, and "demand we couldn't capture" isn't "no opportunity." `score.py`/`dashboard.py` flag `measured = views > 0 or brand_views == 0`, so the score stops conflating the two.*
...
- **Pickle's on-site search is fuzzy** — a "giggle" search returns *ski goggles*. So I measured supply with **exact style-name matching** — denominator **brand-filtered** (the pages mix in other labels — the Nadine Merabi page is only **38% Nadine Merabi**), numerator matched across the **full page** to recover listings with mis-parsed brand fields (typos, "SOLD OUT" prefixes) — plus **scroll-until-stable** to beat the ~280-item scroll cap → "0 of 693 / 76 of 724 / 6 of 231" is catalog-wide, not a sample.
- **Momentum diff is guarded, not asserted.** Pickle's brand pages return a *rotating subset* — two same-depth scrapes share only ~33% of listings — so a naive UUID-diff over-reports turnover badly (I measured 53–67% "disappeared" with total counts *flat*, which can't be real rentals). `diff.py`'s comparability guard suppresses those and reports turnover only where snapshots overlap ≥70% (Nadine Merabi's 1-day pair passed at ~90% → a modest 10%, though 60-gone/61-new against a flat 602→603 total is close to the rotation noise floor, so I'd treat even that as directional). Trustworthy rental velocity needs a stable per-listing ID or the inventory feed, not the paginated page.
